...
from ..ImportTools import get_resolution
import re
...
def correct_mzML_ID(mzmlpath, output_path=None):
    """
    ...
    if output_path is None:
        base, _ = os.path.splitext(os.fspath(mzmlpath))
        output_path = base + "_corrected.mzML"
    with open(mzmlpath, 'r', encoding="utf-8") as fin, open(output_path, 'w', encoding="utf-8") as fout:
        for line in fin:
...
class MZMLImporter(Importer):
    ...
    def init_scans(self):
        ...
        if len(np.unique(self.scans)) < len(self.scans) and len(self.scans) > 1:
            ...
            self._reader_path = corrected_path
    ...
    def _get_spectrum(self, scan):
        """Get a spectrum by ID, falling back to sequential access for nonstandard IDs."""
        try:
            return self.msrun[int(scan)]
        except Exception as direct_error:
            self.reset_reader()
            target = str(scan)
            found = None
            for spectrum in self.msrun:
                if getattr(spectrum, "ms_level", None) is not None and str(spectrum.ID) == target:
                    found = spectrum
                    break
            self.reset_reader()
            if found is None:
                raise KeyError(f"spectrum {scan!r} was not found") from direct_error
            return found
    ...
    def get_inj_time_array(self):
        ...
        for i, spectrum in enumerate(self.msrun):
            if getattr(spectrum, "ms_level", None) is None or str(spectrum.ID) not in scan_ids:
                continue
            it = get_inj_time(spectrum)
            try:
                it = float(it)
            except:
                print("Error in scan header:", i, spectrum.ID, it)
                it = 1
            its.append(it)
        self.reset_reader()
        return np.array(its)
